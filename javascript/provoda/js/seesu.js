...
'js/modules/PlayerSeesu', 'js/models/invstg', 'cache_ajax', 'js/libs/ProspApi', 'js/libs/VkRawSearch'],
...
AppModel.extendTo(SeesuApp, {
	...
	init: function(version){
		...
		var lu = app_serv.store('app-usage-last');
		...
		setInterval(function(){
			...
			if ((now - _this.last_usage)/ (1000 * 60 * 60) > 4){
				_this.checkStats();
				app_serv.store('app-usage-last', (_this.last_usage = new Date()).toUTCString(), true);
				app_serv.store('app-usage-counter', ++_this.usage_counter, true);
			}
		...
		setTimeout(function() {
			...
			var last_ver = app_serv.store('last-app-ver');
			_this.migrateStorage(last_ver);
			app_serv.store('last-app-ver', version, true);
...
(function(){
	...
	if (app_env.cross_domain_allowed) {
		...
		su.mp3_search.add(new VkRawSearch({
